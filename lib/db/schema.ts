...
// ─── 市场配置表 ───────────────────────────────────────────────────────────────
export const markets = pgTable('mf_nv_markets', {
  market_code: varchar('market_code', { length: 10 }).primaryKey(),
  country_name: varchar('country_name', { length: 100 }).notNull(),
  currency: varchar('currency', { length: 10 }).notNull(),
  drive_side: varchar('drive_side', { length: 5 }),
  climate_zone: varchar('climate_zone', { length: 20 }),
  active: boolean('active').default(false),
})

// ─── 品牌表 ──────────────────────────────────────────────────────────────────
export const brands = pgTable('mf_nv_brands', {
...
// ─── 车型表 ──────────────────────────────────────────────────────────────────
export const models = pgTable('mf_nv_models', {
  ...
  steering: varchar('steering', { length: 5 }),
...
// ─── 故障码表 ─────────────────────────────────────────────────────────────────
export const dtcs = pgTable('mf_nv_dtcs', {
  dtc_id: serial('dtc_id').primaryKey(),
  dtc_code: varchar('dtc_code', { length: 20 }).notNull(),
  dtc_type: varchar('dtc_type', { length: 20 }).default('STANDARD'),
...
// ─── 故障码-车型关联表 ────────────────────────────────────────────────────────
export const dtcModelNotes = pgTable('mf_nv_dtc_model_notes', {
  id: serial('id').primaryKey(),
  dtc_id: integer('dtc_id').references(() => dtcs.dtc_id),
  ...
  likely_causes: json('likely_causes'),
...
// ─── 软件更新追踪表 ───────────────────────────────────────────────────────────
export const softwareUpdates = pgTable('mf_nv_software_updates', {
  update_id: serial('update_id').primaryKey(),
  model_id: varchar('model_id', { length: 100 }).references(() => models.model_id),
  market_code: varchar('market_code', { length: 10 }).references(() => markets.market_code),
  version: varchar('version', { length: 50 }).notNull(),
  release_date: varchar('release_date', { length: 30 }),
  update_method: varchar('update_method', { length: 20 }),
...
// ─── 服务费用表 ───────────────────────────────────────────────────────────────
export const serviceCosts = pgTable('mf_nv_service_costs', {
  cost_id: serial('cost_id').primaryKey(),
  model_id: varchar('model_id', { length: 100 }).references(() => models.model_id),
  market_code: varchar('market_code', { length: 10 }).references(() => markets.market_code),
  service_type: varchar('service_type', { length: 100 }).notNull(),
  cost_min: integer('cost_min'),
  cost_max: integer('cost_max'),
...
// ─── 经销商/服务网点表 ────────────────────────────────────────────────────────
export const dealers = pgTable('mf_nv_dealers', {
...
// ─── 案例表 ──────────────────────────────────────────────────────────────────
export const cases = pgTable('mf_nv_cases', {
...
// ─── 案例-故障码关联表 ────────────────────────────────────────────────────────
export const caseDtcLinks = pgTable('mf_nv_case_dtc_links', {
  case_id: integer('case_id').references(() => cases.case_id).notNull(),
...
// ─── 案例媒体附件表 ───────────────────────────────────────────────────────────
export const caseMedia = pgTable('mf_nv_case_media', {
  media_id: serial('media_id').primaryKey(),
  case_id: integer('case_id').references(() => cases.case_id),
...
// ─── 待审队列表 ────────────────────────────────────────────────────────────────
export const pendingCases = pgTable('mf_nv_pending_cases', {
  id: serial('id').primaryKey(),
...
export const pendingSoftwareUpdates = pgTable('mf_nv_pending_software_updates', {
  id: serial('id').primaryKey(),
  raw_content: text('raw_content').notNull(),
  source_platform: varchar('source_platform', { length: 100 }).notNull(),
  source_url: text('source_url'),
...
export const pendingServiceCosts = pgTable('mf_nv_pending_service_costs', {
  id: serial('id').primaryKey(),
...
// ─── 警告灯表 ──────────────────────────────────────────────────────────────────
export const warningLights = pgTable('mf_nv_warning_lights', {
  id: serial('id').primaryKey(),
  ...
  causes: json('causes'),
  can_drive: varchar('can_drive', { length: 20 }),
